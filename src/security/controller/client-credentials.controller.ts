...
import { ApiBearerAuth, ApiCreatedResponse, ApiOperation, ApiBody, ApiUnauthorizedResponse, ApiTags, ApiOkResponse, ApiNotFoundResponse, ApiParam } from '@nestjs/swagger';
...
@ApiBearerAuth()
@Controller(`${Constants.API_PREFIX}/${Constants.API_VERSION_1}/${Constants.CLIENT_CREDENTIALS_ENDPOINT}`,)
export class ClientCredentialsController {
  ...
  @Get()
  @HttpCode(200)
  @ApiOperation({ summary: 'Get Client Credentials', description: 'Get all Client Credentials' })
  @ApiOkResponse({ type: NewClientCredentialsDTO, isArray: true, description: 'All Client Credentials' })
  ...
  @Get(":id")
  @HttpCode(200)
  @ApiOperation({ summary: 'Get Client Credential By Id', description: 'Get Client Credential by Id' })
  @ApiOkResponse({ type: NewClientCredentialsDTO, isArray: false, description: 'Get Client Credential by Id' })
  @ApiUnauthorizedResponse({
    description:
      'thrown if there is not an authorization token or if authorization token does not have enough privileges',
  })
  ...
  @Post()
  @HttpCode(201)
  @ApiCreatedResponse({ type: NewClientCredentialsDTO, description: 'Credential created' })
  ...
  public async updateClientCredential(
    @Headers('authorization') authorization: string,
    @Param('id') id:ClientCredentials['id'], @Body() updateInfo: UpdateClientCredentialsDTO,
  ): Promise<ClientCredentialsDTO> {
    if (!authorization) {
      throw new UnauthorizedException();
    }
