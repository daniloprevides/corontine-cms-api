...
import ScopeEditComponent from "./scope-group-edit.svelte";
/**
 * Variables
 */
let component = null;
...
export class GroupManagerModel {
  params: any = { page: 1 };
  items: [];
  total: number;
  count: number;
  ...
  createCrudAddModal(modal:any, item?:any) {
    modal.renderer = function(root, dialog) {
      const pageView = document.createElement("scope-group-edit") as any;
      pageView.dialog = modal;
      pageView.scopes = scopes;
      ...
      customElements.whenDefined("scope-group-edit").then(d => {
...
const model = new GroupManagerModel();
